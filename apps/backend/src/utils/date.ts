...
export class DateUtils {
	...
	static getDateStringWithFormat(
		...
		const date = input.date ?? DateUtils.getJSDate();
		const format =
			input.format ?? process.env.DATE_FORMAT ?? DateUtils.DEFAULT_DATE_FORMAT;

		return DateTime.fromJSDate(date, { zone: "utc" })
			.setZone(DateUtils.APP_TIMEZONE)
			.toFormat(format);
	}
	...
	static getJSDate(): Date {
		return DateTime.now().setZone(DateUtils.APP_TIMEZONE).toJSDate();
	}
	...
	static createJSDate({ date, utc = true }: CreateDateInput): Date {
		...
		return dateTime.setZone(DateUtils.APP_TIMEZONE).toJSDate();
	}
	...
	static createISODate({ date, utc = true }: CreateDateInput): string {
		...
		return dateTime.setZone(DateUtils.APP_TIMEZONE).toISO()!;
	}

	static getDate(): DateTime {
		return DateTime.now().setZone(DateUtils.APP_TIMEZONE);
	}
