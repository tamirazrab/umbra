...
import type { SortInput } from "./sort";
import { type Infer, InputValidator } from "./validator";

const maxLimit = (limit: number) => (limit > 100 ? 100 : limit);

export const PaginationSchema = InputValidator.object({
	page: InputValidator.number()
		.or(InputValidator.string())
		.or(InputValidator.nan())
		.default(1),
	limit: InputValidator.number()
		.or(InputValidator.string())
		.or(InputValidator.nan())
		.default(10),
})
...
export type PaginationInput<T> = Infer<typeof PaginationSchema> &
	SortInput &
