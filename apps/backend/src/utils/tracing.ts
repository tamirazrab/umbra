...
const getResource = (): Resource => {
	try {
		...
		logger.error(
			new ApiBadRequestException("Error detecting resources for tracing", {
				originalError: error,
			}),
		);
...
const updateSpanName = (
	...
	if (request instanceof IncomingMessage) {
		const host =
			[headers?.origin, headers?.host].find(Boolean) || "unknown-host";
	...
	const path =
		generalizePath((request as { url?: string })?.url?.split("?")[0] || "/") ||
		"/";
...
const httpInstrumentation = new HttpInstrumentation({
	ignoreIncomingRequestHook: (request: IncomingMessage) => {
		...
		return ["/health", "/favicon.ico", "/api-docs"].some((ignorePath) =>
			path?.startsWith(ignorePath),
		);
	},
	ignoreOutgoingRequestHook: (request: RequestOptions) => {
		const path = request.path;
		return ["/favicon.ico", "/v1/metrics", "/v1/traces"].some((ignorePath) =>
			path?.startsWith(ignorePath),
		);
	},
	responseHook: (span: Span, response: IncomingMessage | ServerResponse) => {
		...
		try {
			const req =
				(response as ServerResponse)?.req ||
			...
			const statusCode =
				response.statusCode || (response as ServerResponse).statusCode;
			if (statusCode) {
				span.setAttribute("http.status_code", statusCode);
			}
		} catch (error) {
			logger.warn({
				message: "Error in HTTP response hook:",
	...
	requestHook: (span: Span, request: ClientRequest | IncomingMessage) => {
		...
		try {
			...
			const url =
				(request as IncomingMessage)?.url || (request as ClientRequest)?.path;
			if (url) {
				span.setAttribute("http.url", url);
			}
		} catch (error) {
			logger.warn({
				message: "Error in HTTP request hook:",
				obj: { originalError: error },
			});
		}
	},
...
const redisInstrumentation = new RedisInstrumentation({
	...
	responseHook: (span: Span) => {
		...
		try {
			...
			logger.warn({
				message: "Error in Redis response hook:",
				obj: { originalError: error },
			});
		}
	},
...
const mongodbInstrumentation = new MongoDBInstrumentation({
	...
	responseHook: (span: Span) => {
		...
		try {
			...
			logger.warn({
				message: "Error in MongoDB response hook:",
				obj: { originalError: error },
			});
		}
	},
...
const pgInstrumentation = new PgInstrumentation({
	...
	responseHook: (span: Span) => {
		...
		try {
			...
			logger.warn({
				message: "Error in PostgreSQL response hook:",
				obj: { originalError: error },
			});
		}
	},
...
const start = (): void => {
	...
	try {
		...
		logger.error(
			new ApiBadRequestException("Tracing start error", {
				originalError: error,
			}),
		);
	}
...
const shutdown = async (): Promise<void> => {
	...
	try {
		...
		logger.error(
			new ApiBadRequestException("Tracing shutdown error", {
				originalError: error,
			}),
		);
		throw error;
...
const getHeaders = (
	...
	if ("getHeader" in request) {
		return (request as ClientRequest).getHeaders() as Record<
			string,
