import OpossumCircuitBreaker, { type Options } from "opossum";
...
export function CircuitBreaker(
	params: CircuitBreakerInput = { options: {}, circuitGroup: "default" },
) {
	return (
		target: object,
		...
		const opt: Options = {
			...
			errorThresholdPercentage:
				params?.options?.errorThresholdPercentage ?? ERROR_THRESHOLD_PERCENTAGE,
			volumeThreshold: params?.options?.volumeThreshold ?? VOLUME_THRESHOLD,
			rollingCountTimeout:
				params?.options?.rollingCountTimeout ?? ROLLING_COUNT_TIMEOUT,
		...
		descriptor.value = async function (...args: unknown[]) {
			...
			if (!breakerInstances.has(instanceKey)) {
				...
				if (
					fallbackMethod &&
					typeof (this as any)[`${fallbackMethod}`] === "function"
				) {
					breaker.fallback(async (args: unknown, error: Error) => {
						// eslint-disable-next-line @typescript-eslint/no-explicit-any
						return await (this as any)[`${fallbackMethod}`]({
							input: args,
							err: error,
						});
					});
