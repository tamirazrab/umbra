import { type FindOperator, In, Raw } from "typeorm";

import type { IEntity } from "@/utils/entity";
import { ApiBadRequestException } from "@/utils/exception";

import { type AllowedFilter, SearchTypeEnum } from "../../types";
...
export function ConvertTypeOrmFilter<T>(
	...
	return (
		...
		descriptor.value = function (
			...args: { search: ConvertTypeOrmFilterInput }[]
		) {
			const input = args[0];
			...
			for (const allowedFilter of allowedFilterList) {
				...
				if (allowedFilter.type === SearchTypeEnum.like) {
					if (typeof filters === IS_ARRAY_FILTER) {
						...
						where[`${field}`] = Raw(
							(alias: string) => createManyLike(alias),
							valueFilter,
						);
					}
