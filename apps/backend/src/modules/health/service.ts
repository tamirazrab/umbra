...
import {
	type DatabaseConnectionOutput,
	type DatabaseMemoryOutput,
	HealthStatus,
	type Load,
} from "./types";
...
export class HealthService implements IHealthAdapter {
	...
	async getMongoConnections(): Promise<DatabaseConnectionOutput> {
		try {
			...
			error = this.buildError(
				error,
				`${HealthService.name}/getMongoConnections`,
			);
			this.logger.error(error as ErrorType);
			return { active: 0, available: 0, current: 0, totalCreated: 0 };
		}
	...
	async getPostgresConnections(): Promise<DatabaseConnectionOutput> {
		try {
			...
			return {
				...
				available:
					Number(available[0].max_connections) - Number(current[0].count),
	...
	getActiveConnections() {
		return new Promise((resolve, reject) => {
			...
			exec(
				`lsof -i -n -p ${process.pid} | grep ESTABLISHED | wc -l`,
				(error, stdout, stderr) => {
					if (error) {
