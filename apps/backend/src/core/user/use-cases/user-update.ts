...
import type { IRoleRepository } from "@/core/role/repository/role";
...
export const UserUpdateSchema = UserEntitySchema.pick({
  ...
  .merge(
    UserEntitySchema.pick({ name: true, email: true, roles: true }).partial(),
  )
  .merge(
    InputValidator.object({
      roles: InputValidator.array(InputValidator.enum(RoleEnum)).optional(),
    }),
  )
  .strict();
...
export class UserUpdateUsecase implements IUsecase {
  ...
  async execute(
    ...
    { tracing, user: userData }: ApiTracingInput,
    ...
    tracing.logEvent(
      "user-updated",
      `user: ${user.email} updated by: ${userData.email}`,
    );
