...
import { mockReturnValue, getUUID, mockResolvedValue, expectZodError, nameOf } from '@/test/mock';
...
describe(ResetPasswordConfirmUsecase.name, () => {
    ...
    const app = await Test.createTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: {
            verify: mockReturnValue<ResetPasswordConfirmVerify>({ id: getUUID() })
          }
        },
        {
    ...
    await expectZodError(
      ...
      (issues: ZodExceptionIssue[]) => {
        expect(issues).toEqual([
          { message: 'Required', path: nameOf<ResetPasswordConfirmInput>('token') },
          { message: 'Required', path: nameOf<ResetPasswordConfirmInput>('password') },
          { message: 'Required', path: nameOf<ResetPasswordConfirmInput>('confirmPassword') }
        ]);
      }
    );
  });
  ...
  test('when user not found, should expect an error', async () => {
    userRepository.findOneWithRelation = mockResolvedValue<UserEntity>(null);

    await expect(usecase.execute(input)).rejects.toThrow(ApiNotFoundException);
  });

  test('when token was expired, should expect an error', async () => {
    userRepository.findOneWithRelation = mockResolvedValue<UserEntity>(user);
  ...
  const defaultResetPassword = new ResetPasswordEntity({ id: getUUID(), token: 'token', user });
  test('when confirm successfully, should expect a void', async () => {
    userRepository.findOneWithRelation = mockResolvedValue<UserEntity>(user);
    userRepository.create = mockResolvedValue<CreatedModel>();
    repository.findByIdUserId = mockResolvedValue<ResetPasswordEntity>(defaultResetPassword);
    repository.remove = mockResolvedValue<RemovedModel>();
