...
import { expectZodError, getDate, getUUID, mockResolvedValue, nameOf } from 'test/mock';
...
describe(CatListUsecase.name, () => {
  ...
  test('when no input is specified, should expect an error', async () => {
    await expectZodError(
      ...
      (issues: ZodExceptionIssue[]) => {
        expect(issues).toEqual([{ message: 'Required', path: nameOf<CatListInput>('search') }]);
      }
    );
  });
  ...
  test('when cats are found, should expect an user list', async () => {
    ...
    repository.paginate = mockResolvedValue<CatListOutput>(output);
