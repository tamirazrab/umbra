...
describe(RoleCreateUsecase.name, () => {
  ...
  test('when no input is specified, should expect an error', async () => {
    await expectZodError(
      ...
      (issues: ZodExceptionIssue[]) => {
        expect(issues).toEqual([{ message: 'Required', path: nameOf<RoleCreateInput>('name') }]);
      }
    );
  });
  ...
  test('when role created successfully, should expect a role created', async () => {
    const output: RoleCreateOutput = { created: true, id: getUUID() };
    repository.create = mockResolvedValue<RoleCreateOutput>(output);
