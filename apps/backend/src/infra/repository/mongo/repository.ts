...
import type { FilterQuery } from "@/utils/mongoose";

import type { IRepository } from "../adapter";
import type {
	CreatedModel,
	...
	implements IRepository<T>
{
	...
	private toObject(
		document: T,
		options: { virtuals?: boolean } = { virtuals: true },
	): T {
	...
	async find<TFil = FilterQuery<T>, TOptions = FilterQuery<IEntity>>(
		filter: TFil,
		options?: TOptions,
	): Promise<T[]> {
		try {
	...
	@ConvertMongoFilterToBaseRepository()
	async findOne<TFil = FilterQuery<T>, TQue = FilterQuery<IEntity>>(
		filter: TFil,
		options?: TQue,
	): Promise<T | null> {
		try {
	...
	@ConvertMongoFilterToBaseRepository()
	async findAll<TFil = FilterQuery<T>, TOpt = FilterQuery<IEntity>>(
		filter?: TFil,
		options?: TOpt,
	): Promise<T[]> {
		try {
	...
	async findOr<TOptions = FilterQuery<IEntity>>(
		...
		try {
			...
			const data = await this.model.find(
				{
					$or: filter as FilterQuery<T>[],
					deletedAt: null,
				} as FilterQuery<IEntity>,
				undefined,
				defaultOptions as FilterQuery<IEntity>,
			);
			return data.map((d) => this.toObject(d));
