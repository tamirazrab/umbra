...
export class insertRoles1727654843890 implements MigrationInterface {
	entityBackOffice = new RoleEntity({
		id: UUIDUtils.create(),
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.manager.insert(
			RoleSchema,
			this.entityBackOffice as QueryDeepPartialEntity<RoleSchema>,
		);
		await queryRunner.manager.insert(
			RoleSchema,
			this.entityUser as QueryDeepPartialEntity<RoleSchema>,
		);
	}

	public async down(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.manager.delete(RoleSchema, {
			id: this.entityBackOffice.id,
		});
