...
export class insertUser1727655177319 implements MigrationInterface {
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.manager.insert(
			UserPasswordSchema,
			password as QueryDeepPartialEntity<UserPasswordSchema>,
		);
		...
		await queryRunner.manager.insert(
			UserSchema,
			entity as QueryDeepPartialEntity<UserSchema>,
		);

		for (const role of roles) {
			await queryRunner.query(
				`INSERT INTO users_roles (users_id, roles_id) VALUES('${entity.id}', '${role.id}');`,
			);
		}
